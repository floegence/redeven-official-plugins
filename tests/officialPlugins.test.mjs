...
describe('official plugin repository contract', () => {
  it('keeps Containers source, icon, and generated client in this repository', async () => {
    ...
    assert.equal(containers.manifest.plugin.version, '4.4.6');
  ...
  it('keeps the release train and current catalog aligned at Containers 4.4.6', async () => {
    const [containers] = await loadAllPluginSources(repoRoot);
    assert.equal(containers.release.channel, 'stable');
    assert.equal(containers.release.source_version, '4.4.6');
    assert.equal(containers.release.release_train_tag, 'v4.4.6');
    assert.equal(containers.release.previous_release_train_tag, 'v4.4.5');
    assert.equal(containers.release.stable_catalog.version, '4.4.6');
    ...
    assert.equal(catalog.plugins[0].latest.version, '4.4.6');
    ...
    assert.deepEqual(catalog.plugins[0].latest.distribution, {
      ...
      tag: 'v4.4.6',
      artifact_name: 'containers-4.4.6.redevplugin',
      release_ref_asset_name: 'containers-4.4.6.release-ref.json',
