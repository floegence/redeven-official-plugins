...
describe('official plugin repository contract', () => {
  it('keeps Containers source, icon, and generated client in this repository', async () => {
    ...
    assert.equal(containers.manifest.plugin.version, '4.4.0');
  ...
  it('keeps the release train and current catalog aligned at Containers 4.4.0', async () => {
    const [containers] = await loadAllPluginSources(repoRoot);
    assert.equal(containers.release.channel, 'stable');
    assert.equal(containers.release.source_version, '4.4.0');
    assert.equal(containers.release.release_train_tag, 'v4.4.0');
    assert.equal(containers.release.previous_release_train_tag, 'v4.1.0');
    assert.equal(containers.release.stable_catalog.version, '4.4.0');
    ...
    assert.equal(catalog.plugins[0].latest.version, '4.4.0');
    ...
    assert.deepEqual(catalog.plugins[0].latest.distribution, {
      ...
      tag: 'v4.4.0',
      artifact_name: 'containers-4.4.0.redevplugin',
      release_ref_asset_name: 'containers-4.4.0.release-ref.json',
  ...
  it('keeps packages out of git and uses neutral external signer exchange', async () => {
    const [rootPackage, packageScript, releaseScript, publisherConfig, capabilityPin, capabilitySource, capabilityStageScript, publishScript, buildScript, releaseWorkflow, recoveryWorkflow, tracked] = await Promise.all([
      ...
      readFile(path.join(repoRoot, 'scripts', 'build_official_plugin.mjs'), 'utf8'),
      readFile(path.join(repoRoot, 'scripts', 'release_official_plugin.mjs'), 'utf8'),
      readJSON(path.join(repoRoot, 'releases', 'containers', '4.4.0', 'publisher-config.json')),
      readJSON(path.join(repoRoot, 'plugins', 'containers', 'host-capability.pin.json')),
      readJSON(path.join(repoRoot, 'releases', 'containers', '4.4.0', 'capability-source.json')),
      readFile(path.join(repoRoot, 'scripts', 'stage_release_capability.mjs'), 'utf8'),
