...
describe('official plugin repository contract', () => {
  it('keeps Containers source, icon, and generated client in this repository', async () => {
    ...
    assert.equal(containers.manifest.plugin.version, '4.4.7');
  ...
  it('keeps the release train and current catalog aligned at Containers 4.4.7', async () => {
    ...
    assert.equal(containers.release.stable_catalog.version, '4.4.7');
    ...
    assert.equal(catalog.plugins[0].latest.version, '4.4.7');
    ...
    assert.deepEqual(catalog.plugins[0].latest.distribution, {
      ...
      tag: 'v4.4.7',
      artifact_name: 'containers-4.4.7.redevplugin',
