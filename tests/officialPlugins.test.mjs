...
describe('official plugin repository contract', () => {
  it('publishes Weather and Mind Map as current official plugin sources', async () => {
    ...
    assert.equal(mindMap.manifest.plugin.version, '1.0.28');
  ...
  it('resolves each global release tag to exactly one plugin', async () => {
    ...
    assert.equal(resolvePluginForReleaseTag(sources, 'v1.0.28').name, 'mind-map');
  ...
  it('keeps the reusable plugin packaging and release framework', async () => {
    ...
    assert.deepEqual([...new Set(mindMapReleaseInputs)], ['1.0.17', '1.0.18', '1.0.19', '1.0.20', '1.0.21', '1.0.22', '1.0.23', '1.0.24', '1.0.25', '1.0.26', '1.0.27', '1.0.28']);
  });
});
